...
class BaseMarket(object):
    ...
    @classmethod
    def _fix_kline_pd_se(cls, kl_df:pd.DataFrame, n_folds, start=None, end=None):
        """
    ...
    @classmethod
    def _fix_kline_pd_zero(cls, kl_df:pd.DataFrame):
        """
        ...
        if kl_df is None:return

        def fix_zero(trade_day:pd.Series):
        ...
        kl_df:pd.DataFrame = kl_df.apply(fix_zero, axis=1)
        return kl_df
...
class StockBaseMarket(BaseMarket):
    ...
    @classmethod
    def _fix_kline_pd(cls, kl_df:pd.DataFrame, n_folds, start=None, end=None):
        """修复kline接口的返回金融时间序列"""

...
def _benchmark(df:pd.DataFrame, benchmark, symbol:Symbol)->pd.DataFrame:
    """
...
def make_kl_df(symbol, data_mode=ABuEnv.EMarketDataSplitMode.E_DATA_SPLIT_SE,
               ...
               show_progress=True, parallel=False, parallel_save=True)->pd.DataFrame:
    """
    外部获取金融时间序列接口
...
def combine_pre_kl_pd(kl_pd:pd.DataFrame, n_folds=1):
    """
...
def get_n_year(kl_pd:pd.DataFrame, from_year, get_year=1, how='ff'):
    """
